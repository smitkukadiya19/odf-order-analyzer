...
import { Upload, FileText, Download, Trash2, AlertCircle, CheckCircle, Eye, Menu, X } from 'lucide-react';
...
const PDFOrderAnalyzer = () => {
  ...
  const exportToExcel = () => {
    ...
    // Save file
    const fileName = `order-analysis-mobile-${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
        <div className="bg-white rounded-lg shadow-lg">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm sm:text-base text-gray-600">
                  Upload shipping label PDFs to extract order data
                </p>
              </div>
              ...
              <button
                ...
                className="sm:hidden p-2 rounded-lg bg-gray-100 text-gray-600"
              >
          ...
          <div className="p-4 sm:p-6 ">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-6 lg:p-8 mb-4 sm:mb-6 text-center">
              <Upload className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-gray-400 mx-auto mb-2 sm:mb-4" />
              <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-1 sm:mb-2">Upload PDF Files</h3>
              <p className="text-sm text-gray-500 mb-3 sm:mb-4 px-2">
                Select PDF shipping labels to extract order data
              </p>
              <input
                type="file"
                multiple
              ...
              <label
                htmlFor="file-upload"
                className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-lg cursor-pointer inline-flex items-center gap-2 transition-colors text-sm sm:text-base touch-manipulation"
              >
            ...
            {uploadedFiles.length > 0 && (
              <div className="mb-4 sm:mb-6 ">
                ...
                <div className="bg-gray-50 rounded-lg p-3 sm:p-4 max-h-48 sm:max-h-56 overflow-y-auto">
                  {uploadedFiles.map((file, index) => (
                    <div key={index} className="flex items-center justify-between py-2 sm:py-3 border-b border-gray-200 last:border-b-0">
                      ...
                      <button
                        ...
                        className="text-red-500 hover:text-red-700 p-2 touch-manipulation flex-shrink-0"
                      >
            ...
            {/* Action Buttons - Mobile optimized with stacked layout */}
            <div className={`mb-4 sm:mb-6 ${showMobileMenu || window.innerWidth >= 640 ? 'block' : 'hidden sm:block'}`}>
              {/* Primary actions - always visible */}
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 sm:gap-3 mb-3 sm:mb-0">
                <button
                  onClick={processAllPDFs}
                  disabled={uploadedFiles.length === 0 || isProcessing}
                  className="w-full sm:w-auto bg-green-500 hover:bg-green-600 disabled:bg-gray-300 text-white px-4 py-3 sm:px-6 sm:py-2 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm sm:text-base touch-manipulation"
                >
                  {isProcessing ? (
                    <>
                ...
                <button
                  ...
                  className="w-full sm:w-auto bg-blue-500 hover:bg-blue-600 disabled:bg-gray-300 text-white px-4 py-3 sm:px-6 sm:py-2 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm sm:text-base touch-manipulation"
                >
                ...
                <button
                  ...
                  disabled={allProductsData.length === 0}
                  className="w-full sm:w-auto bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-300 text-white px-4 py-2 sm:px-6 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm touch-manipulation"
                >
                  <Eye className="w-4 h-4" />
                  {showQuantitySummary ? 'Hide' : 'Show'} Summary
                </button>

                <button
                  onClick={() => setShowAllOrders(!showAllOrders)}
                  disabled={allProductsData.length === 0}
                  className="w-full sm:w-auto bg-teal-500 hover:bg-teal-600 disabled:bg-gray-300 text-white px-4 py-2 sm:px-6 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm touch-manipulation"
                >
                  <Eye className="w-4 h-4" />
                ...
                <button
                  onClick={clearAll}
                  className="w-full sm:w-auto bg-red-500 hover:bg-red-600 text-white px-4 py-2 sm:px-6 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm touch-manipulation"
                >
                  <Trash2 className="w-4 h-4" />
                  Clear All
                </button>
            ...
            {isProcessing && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-blue-700 font-semibold text-sm sm:text-base">Processing PDFs...</span>
                </div>
                <div className="text-xs sm:text-sm text-blue-600 bg-white p-2 sm:p-3 rounded border break-words">
                  {processingStatus}
                </div>
              </div>
            )}
            ...
            {!isProcessing && allProductsData.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-green-700 font-semibold text-sm sm:text-base">Processing Complete!</span>
                </div>
            ...
            {allProductsData.length > 0 && (
              <div className="mb-4 sm:mb-6">
                <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-3 sm:mb-4">Extraction Results</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-6">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-blue-800 text-xs sm:text-sm">Total Orders</h4>
                    <p className="text-lg sm:text-xl lg:text-2xl font-bold text-blue-600">{allProductsData.length}</p>
                  </div>
                  <div className="bg-green-50 border border-green-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-green-800 text-xs sm:text-sm">Unique Orders</h4>
                  ...
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-purple-800 text-xs sm:text-sm">Unique SKUs</h4>
                  ...
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-orange-800 text-xs sm:text-sm">Unique Sizes</h4>
                    <p className="text-lg sm:text-xl lg:text-2xl font-bold text-orange-600">
                      {new Set(allProductsData.map(p => p.size)).size}
                    </p>
                  </div>
                  <div className="bg-pink-50 border border-pink-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-pink-800 text-xs sm:text-sm">Total Qty</h4>
                    <p className="text-lg sm:text-xl lg:text-2xl font-bold text-pink-600">
                      {allProductsData.reduce((sum, p) => sum + p.quantity, 0)}
                    </p>
                  </div>
                  <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-2 sm:p-3 lg:p-4">
                    <h4 className="font-semibold text-indigo-800 text-xs sm:text-sm">PDFs</h4>
                    <p className="text-lg sm:text-xl lg:text-2xl font-bold text-indigo-600">{processedData.length}</p>
                  </div>
                </div>

                {/* Quantity Summary by Color and Size - Mobile optimized */}
                {showQuantitySummary && (
                  <div className="mb-4 sm:mb-6">
                    <h4 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-sm sm:text-base">
                      Quantity Summary by Color & Size
                    </h4>
                              ...
                              <div key={index} className="bg-white rounded-lg border p-3 sm:p-4 hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-center mb-2 sm:mb-3">
                                  <div className="flex items-center gap-2">
                                    ...
                                    ></div>
                                    <span className="font-semibold text-gray-800 capitalize text-sm sm:text-base truncate">
                                      {entry.color}
                                    </span>
                                  </div>
                                  <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs sm:text-sm font-semibold whitespace-nowrap">
                                    Size {entry.size}
                                  </span>
                                </div>
                                ...
                                {/* Show individual orders for this color/size - Mobile optimized */}
                                <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-gray-200">
                                  <details className="cursor-pointer">
                                    <summary className="text-xs sm:text-sm text-blue-600 hover:text-blue-800 font-medium touch-manipulation">
                                    ...
                                    <div className="mt-2 space-y-2 max-h-32 sm:max-h-40 overflow-y-auto">
                                      {entry.orders.map((order, orderIndex) => (
                                        <div key={orderIndex} className="bg-gray-50 rounded p-2 text-xs">
                                          <div className="font-mono text-gray-600 mb-1 truncate">
                ...
                {/* Color-wise Summary - Mobile optimized */}
                <div className="mb-4 sm:mb-6">
                  <h4 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-sm sm:text-base">
                    Color-wise Total Summary
                  </h4>
                  <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                    {(() => {
                      // Group by color only
                      ...
                      return (
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
                          {Array.from(colorMap.values()).map((colorData, index) => (
                            <div key={index} className="bg-white rounded-lg border p-3 sm:p-4 text-center">
                              <div className="flex justify-center items-center gap-2 mb-2">
                                <div 
                                  className="w-4 h-4 sm:w-6 sm:h-6 rounded-full border-2 flex-shrink-0"
                                  style={{
                ...
                {/* All Orders Table - Mobile optimized with horizontal scroll */}
                {showAllOrders && (
                  <div className="mb-4 sm:mb-6">
                    <h4 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-sm sm:text-base">
                          ...
                          <div key={index} className="bg-white border rounded-lg p-3 shadow-sm">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div>
                                ...
                                <div className="text-gray-800 truncate">{item.productName}</div>
                              </div>
                              <div>
                                <span className="text-gray-500">Size:</span>
                                <div className="text-gray-800">{item.size}</div>
                              </div>
                              <div>
                                <span className="text-gray-500">Color:</span>
                                <div className="text-gray-800 capitalize">{item.color}</div>
                              </div>
                              <div>
                                <span className="text-gray-500">File:</span>
                                <div className="text-gray-600 truncate">{item.fileName}</div>
                              </div>
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="hidden sm:block overflow-x-auto">
                      <table className="w-full border border-gray-200 rounded-lg overflow-hidden">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">Order Number</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">SKU</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">Product</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">Size</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">Color</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">Qty</th>
                            <th className="px-3 py-2 sm:px-4 sm:py-3 text-left font-semibold text-gray-700 text-sm">File</th>
                          </tr>
                        </thead>
                        <tbody>
                          {allProductsData.map((item, index) => (
                            <tr key={index} className="border-t border-gray-200 hover:bg-gray-50">
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-800 font-mono text-xs">
                                <div className="truncate max-w-32">{item.orderNumber}</div>
                              </td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-800 font-mono text-sm">
                                <div className="truncate max-w-24">{item.sku}</div>
                              </td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-800 text-sm">
                                <div className="truncate max-w-32">{item.productName}</div>
                              </td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-600 text-sm">{item.size}</td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-600 capitalize text-sm">{item.color}</td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 font-semibold text-blue-600">{item.quantity}</td>
                              <td className="px-3 py-2 sm:px-4 sm:py-3 text-gray-500 text-xs">
                                <div className="truncate max-w-24">{item.fileName}</div>
                              </td>
                            </tr>
                          ))}
                        </tbody>
            ...
            {/* Fixed Instructions - Mobile optimized */}
            <div className="mt-6 sm:mt-8 bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
              <div className="flex items-start gap-2">
                <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5 text-green-600 mt-0.5 flex-shrink-0" />
                <div>
                  <h4 className="font-semibold text-green-800 mb-2 text-sm sm:text-base">Fixed Issues:</h4>
                  <ul className="text-xs sm:text-sm text-green-700 space-y-1">
                    ...
                    <li>• <strong>Mobile Responsive:</strong> Optimized for all screen sizes and touch devices</li>
                  </ul>
                </div>
              </div>
            </div>
